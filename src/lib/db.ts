...
export async function updateUser(
  id: string,
  input: { name?: string; email?: string; country?: string | null; passwordHash?: string }
): Promise<DBUser> {
  if (prisma) {
...
export async function getAllMicroCredentials(): Promise<DBMicroCredential[]> {
  if (prisma) {
    return prisma.microCredential.findMany({ orderBy: { code: "asc" }, omit: { imageData: true, imageMime: true } });
  }
  const supabase = getSupabase();
  if (!supabase) throw new Error("No database configured.");
  const { data, error } = await supabase
...
export async function getMicroCredentialById(
  ...
  if (prisma) {
    return prisma.microCredential.findUnique({ where: { id }, omit: { imageData: true, imageMime: true } });
  }
  const supabase = getSupabase();
  if (!supabase) throw new Error("No database configured.");
  const { data } = await supabase
...
export async function createMicroCredential(input: {
  ...
  if (prisma) {
    return prisma.microCredential.create({ data: input, omit: { imageData: true, imageMime: true } });
  }
  const supabase = getSupabase();
  if (!supabase) throw new Error("No database configured.");
  const { data, error } = await supabase
...
export async function updateMicroCredential(
  ...
  if (prisma) {
    return prisma.microCredential.update({ where: { id }, data: input, omit: { imageData: true, imageMime: true } });
  }
  const supabase = getSupabase();
  if (!supabase) throw new Error("No database configured.");
...
export async function getAllMicroProgrammes(): Promise<DBMicroProgramme[]> {
  if (prisma) {
    return prisma.microProgramme.findMany({
      orderBy: { code: "asc" },
      omit: { imageData: true, imageMime: true },
      include: {
        credentials: {
          orderBy: { order: "asc" },
          include: { credential: { omit: { imageData: true, imageMime: true } } },
        },
      },
    }).then((programmes: any[]) =>
...
export async function createMicroProgramme(input: {
  ...
  if (prisma) {
    return prisma.microProgramme.create({ data: input, omit: { imageData: true, imageMime: true } });
  }
  const supabase = getSupabase();
  if (!supabase) throw new Error("No database configured.");
  const { data, error } = await supabase
...
export async function updateMicroProgramme(
  ...
  if (prisma) {
    return prisma.microProgramme.update({ where: { id }, data: input, omit: { imageData: true, imageMime: true } });
  }
  const supabase = getSupabase();
  if (!supabase) throw new Error("No database configured.");
  const { data, error } = await supabase
...
export async function getUserEnrolledProgrammes(userId: string): Promise<DBMicroProgramme[]> {
  if (prisma) {
    ...
    return enrollments.map((e: any) => ({
      ...
      imageData: undefined,
      imageMime: undefined,
      hasImage: !!e.programme.imageData,
      credentials: e.programme.credentials.map((pc: any) => ({
        ...pc.credential,
        imageData: undefined,
        imageMime: undefined,
...
export async function getUserEnrolledCredentials(userId: string): Promise<DBMicroCredential[]> {
  if (prisma) {
    ...
    return enrollments.map((e: any) => ({
      ...
      imageData: undefined,
      imageMime: undefined,
      hasImage: !!e.credential.imageData,
    }));
  }
  const supabase = getSupabase();
  if (!supabase) throw new Error("No database configured.");

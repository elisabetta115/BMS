...
export default function Home() {
  ...
  function scrollCarousel(dir: "left" | "right") {
    ...
    carouselRef.current.scrollBy({ left: dir === "left" ? -364 : 364, behavior: "smooth" });
  }
  ...
  useEffect(() => {
    fetch("/api/micro-programmes")
      .then((r) => r.json())
      .then((data) => {
        const mapped: Programme[] = (data.programmes || []).map((p: any) => ({
          id: p.id,
          slug: p.slug,
          title: p.title,
          code: p.code,
          ...
          credentials: (p.credentials || []).map((c: any) => c.title),
        }));
              ...
              <>
                <div className="relative">
                  <button
                    ...
                    className="absolute left-0 top-1/2 -translate-y-1/2 -translate-x-4 z-10 w-10 h-10 rounded-full shadow-md flex items-center justify-center transition-opacity hover:opacity-80"
                  ...
                  <div className="programme-carousel" ref={carouselRef}>
                    {programmes.map((p) => (
                  ...
                  <button
                    ...
                    className="absolute right-0 top-1/2 -translate-y-1/2 translate-x-4 z-10 w-10 h-10 rounded-full shadow-md flex items-center justify-center transition-opacity hover:opacity-80"
                    style={{ background: "var(--bms-green)" }}
                  >
        ...
        <section className="py-16 md:py-20">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div className="grid md:grid-cols-2 gap-12 items-start">
              ...
              <div>
                <p className="text-sm font-semibold italic mb-3" style={{ color: "var(--bms-green)" }}>Certifications to boost your skills and career prospects</p>

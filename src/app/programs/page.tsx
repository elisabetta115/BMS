...
export default function ProgramsPage() {
  ...
  const filtered = useMemo(() => {
    ...
    return programmes.filter(p => p.title.toLowerCase().includes(q) || p.code.toLowerCase().includes(q) || p.project.toLowerCase().includes(q) || (p.credentials || []).some(c => c.title.toLowerCase().includes(q)));
  }, [programmes, search]);
              ...
              {filtered.map(p => (
                <div key={p.id} className="programme-card bg-white rounded-2xl shadow-md overflow-hidden flex flex-col">
                  ...
                  <div className="p-5 flex flex-col flex-1">
                    ...
                    <Link href={`/programs/${p.id}`} className="inline-flex items-center gap-2 text-sm font-semibold text-white px-5 py-2.5 rounded-full self-start" style={{ background: "var(--bms-green)" }}>
